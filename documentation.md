...
1.  **Event Handling (User Input):**
    ...
    *   These handlers determine the context of the event (e.g., which widget was clicked, which key was pressed, new window size). The handlers determine the context of the event primarily through spatial location for mouse events and focus state for keyboard events.
        * For mouse events (handleMouseDown, handleMouseMove, handleMouseUp, handleWheel), the handler first converts the browser's pixel coordinates into character grid coordinates (mouseX_chars, mouseY_chars) using getMouseCoords_chars(). It then identifies the target widget by calling findWidgetAt(), which iterates through the tWidgets array (from top-most visually down) to see if the character coordinates fall within any widget's boundaries (x, y, w, h). If a widget is found, handleMouseDown further refines the context by calculating the click's position relative to the widget's origin (innerX, innerY) and checks if these relative coordinates correspond to specific interactive elements like the title bar, close button, resize handle, or scrollbar. Subsequent mousemove and mouseup events rely on the interactionState object (set during mousedown) to remember which widget and what type of interaction (drag, resize, select) is in progress.
        * For keyboard events (handleKeyDown), the context is simpler: the handler checks the global activeWidget variable. If a widget has focus (activeWidget is not null), the keyboard event object is passed directly to that specific widget's handleKeyPress() method, allowing the widget to interpret the input according to its own state (e.g., the editor inserts text, the menu navigates). The handleResize event handler understands its context globally, recalculating the entire grid dimensions and then iterating through all widgets to adjust their size and position accordingly.
